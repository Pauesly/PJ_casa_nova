...
window.onload = function() {
  ...
  var id_selecionada = searchParams.get('id');
  ...
  $.getJSON('carrega_dados.php',{}, function(retorno){
    ...
    for(let i=0; i < registros; i++){
      ...
      if(retorno[i]['id'] == id_selecionada){

        // Cria um novo elemento div
        var img_presente = document.getElementById("img_presente");
        var link_compra = document.getElementById("link_compra");

        // Define o atributo class do novo elemento
        img_presente.setAttribute("src", retorno[i]['img']);
        link_compra.setAttribute("href", retorno[i]['link']);
...
btn_confirma.addEventListener("click", function(e) {
  e.preventDefault();
  ...
  if(nome == "" || email == ""){
    ...
    $.getJSON('carrega_dados.php',{}, function(retorno){
      ...
      let cont = 0;
      for(let i=0; i < 4; i++){
        if(retorno[i]['id'] == id_chosen){
          if (retorno[i]['selecionado'] == "s"){
            alert ("Produto já escolhido! Por favor, escolha outro.");
            window.location.href = 'index.html';
            cont++;
            break;
          }
      ...
      if(cont == 0){
        ...
        document.getElementById("txt_confirma").style.display = "";
      }
    });
  }
});
